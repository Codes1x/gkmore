...
### 5. О группе (кто мы) {x}
**📍 Файл:** `src/components/modern-about.tsx`
**📄 Страницы:** `/` (обзор) + `/about` (подробно)
- Миссия: стать надёжным партнёром №1… {x}
- Состав: Sea Family Resort & Spa, Клининг, Лучезарный Резорт {x}
- 3 блока компаний: отели + клининг с кнопкой "Подробнее" → `/clining` {x}
...
**📍 Файл:** `src/components/modern-faq.tsx`
**📄 Страница:** `/` (главная)
- 🤔 "Не понимаю отчётности" → dashboard, детализация, менеджер, первый разбор лично {x}
- 😰 "Боюсь простоев" → 8 каналов продаж, динамические цены, умный овербукинг, 78% загрузка {x}  
- 😤 "Нет контроля" → live-статистика, календарь, фото-отчёты, push-уведомления, видеонаблюдение {x}
- 💸 "Высокие комиссии" → расчёт 30% vs 35-45% самоуправления, калькулятор "до/после" {x}
- 🏠 "Хочу сам пожить в квартире" → 14 дней бесплатно, сезонные блокировки, уведомление за 2 недели {x}
- ⚖️ "Хочу расторгнуть договор" → 3 месяца тест-период, без штрафов, smooth transition, 97% остаются {x}
